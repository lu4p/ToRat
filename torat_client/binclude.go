// Code generated by https://github.com/lu4p/binclude; DO NOT EDIT.

package client

import (
	"github.com/lu4p/binclude"
	"time"
)

var (
	BinFS = &binclude.FileSystem{Files: binclude.Files{
		"../keygen": {
			Filename: "keygen", Mode: 0o20000000777, ModTime: time.Unix(1605396303, 1605396303858601180), Compression: binclude.None,
			Content: []byte(""),
		},
		"../keygen/cert.pem": {
			Filename: "cert.pem", Mode: 0o644, ModTime: time.Unix(1605375766, 1605375766581739106), Compression: binclude.None,
			Content: []byte("-----BEGIN CERTIFICATE-----\\nLuvpGsQoGfbMMIG2\n-----END CERTIFICATE-----\n"),
		},
	}}
)
